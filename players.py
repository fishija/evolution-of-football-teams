            ...
            for row in rows:
                ...
                if players_pseudonim!='' and team1==team_for_project:
                    if section_number==0 or section_number==2:
                        ...
                        player_link = link.get('href')  # Get the href attribute of the anchor tag
                        ...
                        soup_for_players = BeautifulSoup(response_for_players_name.text, 'html.parser')
                        sections_for_player = soup_for_players.find_all('title')
                        players_name = sections_for_player[0].text[sections_for_player[0].text.find(',')+2:sections_for_player[0].text.find(' -')]
...
def save_to_excel(name_of_team, name_of_excel, matches_list):
    ...
    for list_row in matches_list:
        try:
            ...
            worbook.save(f'./Football_team_evolution/{name_of_excel}.csv')
